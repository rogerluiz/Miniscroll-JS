...
return arguments[0];},getZindex:function(target){var topZIndex=0;var zIndex=0;var pos=0;var tags=target.getElementsByTagName('*');for(var i=0;i<tags.length;i++){pos=new Miniscroll.Utils.getCss(tags[i]).position;tags[i].style.position="relative";zIndex=new Miniscroll.Utils.getCss(tags[i]).zIndex;tags[i].style.position=pos;if(zIndex>topZIndex){topZIndex=zIndex;}}
...
{element['on'+type]=null;}},preventDefault:function(event){if(event.preventDefault){event.preventDefault();}else{event.returnValue=false;}},mousewheel:function(element,callback,root){}};Miniscroll.Update=function(){};Miniscroll.Destroy=function(){};Miniscroll.Create=function(scroll){this.scroll=scroll;this.prefix="miniscroll-";this.scrollName="miniscroll";this._scrollSize=new Miniscroll.Point(0,0);this._scrollPos=new Miniscroll.Point(0,0);this._settings=this.scroll.settings;this._topZindex=new Miniscroll.Utils.getZindex(this.scroll.target);return this;};Miniscroll.Create.prototype={init:function(){this.addContainer();this.addTracker();this.addThumb();},addContainer:function(){var typeId=(this.scroll.target.getAttribute("id")!==null)?true:false;var typeClass=(this.scroll.target.getAttribute("class")!==null)?true:false;if(typeId){this.scrollName=this.scroll.target.getAttribute("id");}else if(typeClass){this.scrollName=this.scroll.target.getAttribute("class");}
this.scroll.container=new Miniscroll.Utils.create(this.scroll.target,"div",{"id":this.prefix+this.scrollName,"class":this.prefix+"container"});this._scrollSize.x=(this._settings.scrollbarSize!="auto")?this._settings.scrollbarSize:new Miniscroll.Utils.offset(this.scroll.target).width;this._scrollSize.y=(this._settings.scrollbarSize!="auto")?this._settings.scrollbarSize:new Miniscroll.Utils.offset(this.scroll.target).height;this.scrollPos.x=new Miniscroll.Utils.offset(this.scroll.target).left+(this._scrollSize.x-this._settings.size);this.scrollPos.y=new Miniscroll.Utils.offset(this.scroll.target).top+(this._scrollSize.y-this._settings.size);console.log(this._topZindex);},addTracker:function(){console.log("addTracker");},addThumb:function(){console.log("addThumb");}};(function(){if(window.jQuery){jQuery.fn.miniscroll=function(options){return new Miniscroll.Scroll(this,options);};}}());window.Miniscroll=Miniscroll;})(window,document);if(!Math.trunc){Math.trunc=function trunc(x){return x<0?Math.ceil(x):Math.floor(x);};}
