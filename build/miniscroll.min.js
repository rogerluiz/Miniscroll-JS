...
(function(window,document){"use strict";var root=this;var Miniscroll=Miniscroll||{VERSION:'2.0.0',};Miniscroll.Scroll=function(element,options){root=this;};Miniscroll.Scroll.settings={axis:"y",size:10,sizethumb:"auto",scrollbarSize:"auto",thumbColor:"#e74c3c",trackerColor:"#e6e9ed",isKeyEvent:false,turnOffWheel:false,onScroll:function(){}};Miniscroll.Scroll.prototype.constructor=Miniscroll.Scroll;Miniscroll.Utils={getCss:function(element,property){var result;if(!window.getComputedStyle){if(document.defaultView&&document.defaultView.getComputedStyle){result=document.defaultView.getComputedStyle.getPropertyValue(property);}else{result=(element.currentStyle)?element.currentStyle[property]:element.style[property];}}else{result=window.getComputedStyle(element).getPropertyValue(property);}
return result;},setCss:function(element,arguments){for(var prop in arguments){if(prop==='opacity'){element.style.filter='alpha(opacity='+(arguments[prop]*100)+')';element.style.KhtmlOpacity=arguments[prop];element.style.MozOpacity=arguments[prop];element.style.opacity=arguments[prop];}else{if(prop==="marginLeft"||prop==="marginTop"||prop==="left"||prop==="top"||prop==="width"||prop==="height"){element.style[prop]=(typeof arguments[prop]==="string")?arguments[prop]:arguments[prop]+"px";}else{element.style[prop]=arguments[prop];}}}},get:function(selector){var element=null;var self=this;if(selector===window||selector===document||selector==="body"||selector==="body, html"){return document.body;}
...
return new Miniscroll.Point(posx,posy);}};Miniscroll.Point=function(x,y){if(!(this instanceof Miniscroll.Point)){return new Miniscroll.Point(x,y);}
...
{element['on'+type]=function(event){callback.call(self,event,this);};}},off:function(element,type,callback,r){var self=r;var mousewheel=(/Firefox/i.test(navigator.userAgent))?"DOMMouseScroll":"mousewheel";if(element.addEventListener)
...
{element['on'+type]=null;}},preventDefault:function(event){if(event.preventDefault){event.preventDefault();}else{event.returnValue=false;}},mousewheel:function(element,callback,root){}};(function(){if(window.jQuery){jQuery.fn.miniscroll=function(options){return new Miniscroll.Scroll(this,options);};}}());window.Miniscroll=Miniscroll;})(window,document);if(!Math.trunc){Math.trunc=function trunc(x){return x<0?Math.ceil(x):Math.floor(x);};}
